...
class NPortParser:
    ...
    def get_fund_info(self) -> Dict[str, Any]:
        ...
        if gen_info is not None:
            fund_info.update(
                {
                    "fund_name": self.get_text_safe(
                        gen_info.find("seriesName", self.namespaces)
                    ),
                    "reg_name": self.get_text_safe(
                        gen_info.find("regName", self.namespaces)
                    ),
                    "series_id": self.get_text_safe(
                        gen_info.find("seriesId", self.namespaces)
                    ),
                    "reg_cik": self.get_text_safe(
                        gen_info.find("regCik", self.namespaces)
                    ),
                    "reg_lei": self.get_text_safe(
                        gen_info.find("regLei", self.namespaces)
                    ),
                    "series_lei": self.get_text_safe(
                        gen_info.find("seriesLei", self.namespaces)
                    ),
                    "report_period_end": self.get_text_safe(
                        gen_info.find("repPdEnd", self.namespaces)
                    ),
                    "report_period_date": self.get_text_safe(
                        gen_info.find("repPdDate", self.namespaces)
                    ),
                    "is_final_filing": self.get_text_safe(
                        gen_info.find("isFinalFiling", self.namespaces)
                    ),
                }
            )
        ...
        if fund_info_elem is not None:
            fund_info.update(
                {
                    "total_assets": self.get_text_safe(
                        fund_info_elem.find("totAssets", self.namespaces)
                    ),
                    "total_liabilities": self.get_text_safe(
                        fund_info_elem.find("totLiabs", self.namespaces)
                    ),
                    "net_assets": self.get_text_safe(
                        fund_info_elem.find("netAssets", self.namespaces)
                    ),
                }
            )
        ...
        if header_data is not None:
            fund_info.update(
                {
                    "submission_type": self.get_text_safe(
                        header_data.find("submissionType", self.namespaces)
                    ),
                    "is_confidential": self.get_text_safe(
                        header_data.find("isConfidential", self.namespaces)
                    ),
                }
            )
        ...
        if series_class_info is not None:
            fund_info.update(
                {
                    "class_id": self.get_text_safe(
                        series_class_info.find("classId", self.namespaces)
                    )
                }
            )
    ...
    def get_holdings_data(self) -> List[Dict[str, Any]]:
        ...
        for invst in invst_or_secs:
            ...
            # Financial data
            holding["balance"] = self.get_text_safe(
                invst.find("balance", self.namespaces)
            )
            holding["units"] = self.get_text_safe(invst.find("units", self.namespaces))
            holding["currency"] = self.get_text_safe(
                invst.find("curCd", self.namespaces)
            )
            holding["value_usd"] = self.get_text_safe(
                invst.find("valUSD", self.namespaces)
            )
            holding["percent_value"] = self.get_text_safe(
                invst.find("pctVal", self.namespaces)
            )

            # Classification data
            holding["payoff_profile"] = self.get_text_safe(
                invst.find("payoffProfile", self.namespaces)
            )
            holding["asset_category"] = self.get_text_safe(
                invst.find("assetCat", self.namespaces)
            )
            holding["issuer_category"] = self.get_text_safe(
                invst.find("issuerCat", self.namespaces)
            )
            holding["investment_country"] = self.get_text_safe(
                invst.find("invCountry", self.namespaces)
            )
            holding["is_restricted_security"] = self.get_text_safe(
                invst.find("isRestrictedSec", self.namespaces)
            )
            holding["fair_value_level"] = self.get_text_safe(
                invst.find("fairValLevel", self.namespaces)
            )

            # Security lending information
            sec_lending = invst.find("securityLending", self.namespaces)
            if sec_lending is not None:
                holding["is_cash_collateral"] = self.get_text_safe(
                    sec_lending.find("isCashCollateral", self.namespaces)
                )
                holding["is_non_cash_collateral"] = self.get_text_safe(
                    sec_lending.find("isNonCashCollateral", self.namespaces)
                )
    ...
    def to_dataframes(self) -> tuple[HoldingsRawDF, Dict[str, Any]]:
        ...
        if "percent_value" in holdings_df.columns:
            holdings_df["percent_value"] = (
                pd.to_numeric(holdings_df["percent_value"], errors="coerce") / 100
            )
...
def main():
    ...
    if not holdings_df.empty:
        ...
        top_holdings = holdings_df.nlargest(10, "value_usd")[
            ["name", "value_usd", "percent_value"]
        ]
        print(top_holdings)
