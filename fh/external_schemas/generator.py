...
class JSONSchemaGenerator:
    ...
    def import_schemas_from_file(
        ...
        try:
            ...
            for name, obj in inspect.getmembers(module, inspect.isclass):
                ...
                if hasattr(obj, "__mro__") and any(
                    base.__name__ == "DataFrameModel" for base in obj.__mro__
                ):
    ...
    def pandera_to_json_schema(
        ...
        json_schema = {
            ...
            or f"Schema for {schema_class.__name__}",
    ...
    def generate_schema_file(self, output_file: Optional[str] = None) -> None:
        ...
        for schema_name in combined_schema["definitions"].keys():
            field_count = len(
                combined_schema["definitions"][schema_name].get("properties", {})
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate JSON Schema from internal Pandera schemas"
    )
    parser.add_argument(
        "--output-file", help="Custom output filename (default: combined_schema.json)"
    )
