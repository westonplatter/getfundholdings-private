...
class HoldingsRawSchema(pa.DataFrameModel):
    ...
    other_id_desc: Series[str] = pa.Field(nullable=True, description="Other identifier description")
    
    # Position data
    balance: Series[float] = pa.Field(description="Security balance/shares")
    units: Series[str] = pa.Field(description="Units of measurement (e.g., 'NS' for number of shares)")
    currency: Series[str] = pa.Field(description="Currency code")
    value_usd: Series[float] = pa.Field(description="Market value in USD (can be negative for short positions)")
    percent_value: Series[float] = pa.Field(description="Percentage of total portfolio value (can exceed 1.0 for leveraged positions)")
    
    # Classification data
    payoff_profile: Series[str] = pa.Field(description="Payoff profile (e.g., 'Long', 'Short')")
    ...
    is_non_cash_collateral: Series[str] = pa.Field(description="Non-cash collateral flag")
    is_loan_by_fund: Series[str] = pa.Field(description="Loan by fund flag")
    
    # Loan data
    loan_value: Series[float] = pa.Field(nullable=True, description="Loan value if applicable")
    
    # Metadata
    source_file: Series[str] = pa.Field(description="Source N-PORT XML filename")
    report_period_date: Series[str] = pa.Field(description="N-PORT report period date (YYYY-MM-DD)")
...
class HoldingsEnrichedSchema(HoldingsRawSchema):
    ...
    ticker: Series[str] = pa.Field(nullable=True, description="Ticker symbol from OpenFIGI API")
    enrichment_datetime: Series[datetime] = pa.Field(description="Datetime when ticker enrichment was performed (timezone-aware UTC)")
