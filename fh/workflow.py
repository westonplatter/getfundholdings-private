...
class FundHoldingsWorkflow:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for cik in tqdm(self.config.cik_list, desc="Processing CIKs", unit="CIK"):
            ...
            try:
                ...
                results["failed_ciks"] += 1
                results["cik_results"][cik] = {"success": False, "error": str(e)}

        results["total_execution_time"] = time.time() - start_time

        logger.info(
            f"Workflow completed: {results['successful_ciks']}/{results['total_ciks']} CIKs successful"
        )
        return results
    ...
    def process_cik(self, cik: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Step 2: Collecting N-PORT filings for {len(series_data)} series"
            )
            ...
            for series_id in tqdm(
                series_ids,
                desc=f"Collecting filings for {len(series_ids)} series",
                unit="series",
                leave=False,
            ):
                try:
                    ...
                    if series_filings:
                        filings_file = self.sec_client.save_series_filings(
                            series_filings, series_id, cik=cik
                        )
                        filings_files.append(filings_file)
                        total_filings += len(series_filings)
                except Exception as e:
                    logger.warning(
                        f"Failed to fetch filings for series {series_id}: {e}"
                    )
            ...
            logger.info(
                f"Step 3: Downloading N-PORT XML files for {total_filings} filings"
            )
            ...
            logger.info(
                f"Step 4: Extracting holdings data from {len(xml_files)} XML files"
            )
            holdings_files = self.extract_holdings_data(
                cik, xml_files, series_ticker_map
            )
            if holdings_files:
                ...
                if self.config.enable_ticker_enrichment and self.openfigi_client:
                    ...
                    enriched_files = []
                    for holdings_file in tqdm(
                        holdings_files,
                        desc=f"Enriching {len(holdings_files)} holdings files",
                        unit="file",
                        leave=False,
                    ):
                        enriched_file = self.enrich_holdings(
                            holdings_file, cik, series_ticker_map
                        )
                        if enriched_file:
    ...
    def _create_series_ticker_mapping(self, series_data: List[Dict]) -> Dict[str, str]:
        ...
        logger.info(
            f"Created series to ticker mapping for {len(series_ticker_map)} series"
        )
    ...
    def _download_nport_xmls(self, cik: str, series_ids: List[str]) -> List[str]:
        ...
        for series_id in tqdm(
            series_ids,
            desc=f"Downloading XMLs for {len(series_ids)} series",
            unit="series",
            leave=False,
        ):
            try:
                # Load series filings
                series_filings_data = self.sec_client.load_series_filings(
                    cik, series_id
                )
                ...
                for filing in tqdm(
                    filings,
                    desc=f"Downloading filings for {series_id}",
                    unit="filing",
                    leave=False,
                ):
                    accession_number = filing.get("accession_number")
                    if accession_number:
                        xml_file = self.sec_client.download_and_save_nport(
                            cik, accession_number, series_id
                        )
                        if xml_file:
    ...
    def extract_holdings_data(
        self, cik: str, xml_files: List[str], series_ticker_map: Dict[str, str]
    ) -> List[str]:
        ...
        try:
            ...
            for xml_file in tqdm(
                xml_files,
                desc=f"Extracting holdings from {len(xml_files)} XML files",
                unit="file",
                leave=False,
            ):
                try:
                    if os.path.exists(xml_file):
                        ...
                        if not holdings_df.empty:
                            ...
                            holdings_df.to_csv(
                                holdings_file, index=False, quoting=1
                            )  # QUOTE_ALL
                            holdings_files.append(holdings_file)

                            logger.info(
                                f"Saved {len(holdings_df)} holdings for {fund_ticker} to {holdings_file}"
                            )
    ...
    def enrich_holdings(
        self, holdings_file: str, cik: str, series_ticker_map: Dict[str, str]
    ) -> Optional[str]:
    ...
    def _extract_file_metadata(
        self, holdings_file: str, holdings_df: pd.DataFrame
    ) -> Dict[str, str]:
        ...
        if len(filename_parts) >= 6:
            ...
            return {
                "fund_ticker": (
                    holdings_df.get("fund_ticker", ["unknown"]).iloc[0]
                    if "fund_ticker" in holdings_df.columns and not holdings_df.empty
                    else "unknown"
                ),
                "cik": "unknown",
                "series_id": (
                    holdings_df.get("series_id", ["unknown"]).iloc[0]
                    if "series_id" in holdings_df.columns and not holdings_df.empty
                    else "unknown"
                ),
                "report_date": "unknown",
                "filename": filename,
    ...
    def _enrich_metadata(
        self, enriched_df: pd.DataFrame, metadata: Dict[str, str]
    ) -> pd.DataFrame:
    ...
    def _enrich_tickers(self, enriched_df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Excluding {derivative_mask.sum()} derivative instruments from ticker lookup"
        )
    ...
    def _identify_derivative_instruments(self, enriched_df: pd.DataFrame) -> pd.Series:
        ...
        def is_derivative_instrument(name: str, title: str) -> bool:
            ...
            return any(
                indicator in name_lower or indicator in title_lower
                for indicator in derivative_indicators
            )

        return enriched_df.apply(
            lambda row: is_derivative_instrument(
                row.get("name", ""), row.get("title", "")
            ),
            axis=1,
        )

    def _enrich_tickers_by_cusip(
        self, enriched_df: pd.DataFrame, derivative_mask: pd.Series
    ) -> pd.DataFrame:
        ...
        if missing_ticker_count > 0 and "cusip" in [
            col.lower() for col in enriched_df.columns
        ]:
            logger.info(
                f"{missing_ticker_count} holdings missing tickers, trying CUSIP lookup"
            )
            enriched_df = self.openfigi_client.add_tickers_to_dataframe_by_cusip(
                enriched_df, cusip_column="cusip"
            )
        else:
    ...
    def _enrich_tickers_by_isin(
        ...
        if missing_ticker_count > 0 and "isin" in [
            col.lower() for col in enriched_df.columns
        ]:
            logger.info(
                f"{missing_ticker_count} holdings missing tickers, trying ISIN lookup"
            )
            enriched_df = self.openfigi_client.add_tickers_to_dataframe_by_isin(
                enriched_df, isin_column="isin"
            )
        else:
    ...
    def _log_ticker_enrichment_results(
        self, enriched_df: pd.DataFrame, derivative_mask: pd.Series
    ) -> None:
        ...
        success_rate = (
            (non_derivative_count - missing_ticker_count) / non_derivative_count * 100
            if non_derivative_count > 0
            else 0
        )

        logger.info(
            f"Ticker enrichment complete: {non_derivative_count - missing_ticker_count}/{non_derivative_count} non-derivative holdings have tickers ({success_rate:.1f}%)"
        )
        logger.info(
            f"Excluded {derivative_mask.sum()} derivative instruments (ELNs, swaps, etc.) from ticker lookup"
        )

        if missing_ticker_count > 0:
            logger.warning(
                f"{missing_ticker_count} non-derivative holdings still missing tickers after both CUSIP and ISIN lookup attempts"
            )
            for idx, row in enriched_df[missing_ticker_mask].iterrows():
                logger.warning(
                    f"  - {row['name']}/{row['title']}- ISIN: {row['isin']}, CUSIP: {row['cusip']}"
                )
    ...
    def _enrich_notes(self, enriched_df: pd.DataFrame) -> pd.DataFrame:
        ...
        derivative_mask = self._identify_derivative_instruments(enriched_df)
        enriched_df.loc[
            derivative_mask, "enrichment_notes"
        ...
        enriched_df.loc[
            missing_ticker & ~derivative_mask, "enrichment_notes"
        ] += "missing_ticker; "
    ...
    def _save_enriched_data(
        ...
        logger.info(
            f"Saved enriched holdings for {metadata['fund_ticker']} to {enriched_file}"
        )
        return enriched_file
